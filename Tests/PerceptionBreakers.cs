...
namespace RealPlayTester.Tests.Verification
{
    public class PerceptionBreakers
    {
        ...
        public IEnumerator ISSUE_005_ZOrder_Ambiguity() => ToCoroutine(async () =>
        {
            ...
            // Candidate_2 is LATER in hierarchy, so it should be on top
            btn2Go.transform.SetAsLastSibling();

            // SmartFind should pick the one on top (Candidate_2)
            var found = Tester.Perception.Find("Candidate");
            UnityEngine.Debug.Log($"[ISSUE-005] Found: {found.name}");
        ...
        public IEnumerator ISSUE_007_Incorrect_ScreenRect_WorldCanvas() => ToCoroutine(async () =>
        {
            ...
            // Verify if the rect is reasonably within screen or zero
            // If it uses Camera.main incorrectly, it might be off-screen.
            
            UnityEngine.Debug.Log($"[ISSUE-007] DOM length: {dom.Length}");
            // We need a way to parse and check the rect, but for now we check if it didn't crash
        });
        ...
        public IEnumerator ISSUE_016_AlphaZero_Interactivity() => ToCoroutine(async () =>
        {
            ...
            canvasGroup.alpha = 0f; // Invisible and non-interactable usually

            var affordances = Tester.Perception.GetActionableElements();
            UnityEngine.Debug.Log($"[ISSUE-016] Affordances: {affordances}");
        ...
        public IEnumerator ISSUE_020_RaycastTarget_False_Interactivity() => ToCoroutine(async () =>
        {
            ...
            var affordances = Tester.Perception.GetActionableElements();
            UnityEngine.Debug.Log($"[ISSUE-020] Affordances: {affordances}");
